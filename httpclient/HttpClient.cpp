...
//	FileLog("httpclient", "JNI_OnLoad( httpclient.so JNI_OnLoad )");
//
//	// Get JNI
//	JNIEnv* env;
//	if (JNI_OK != vm->GetEnv(reinterpret_cast<void**> (&env),
//                           JNI_VERSION_1_4)) {
//		FileLog("httpclient", "JNI_OnLoad( could not get JNI env )");
//		return -1;
//	}
//
...
//		FileLog("httpclient", "JNI_OnLoad( curl_version : %s )", data->version);
//	}
//
//	if( data->ssl_version != NULL ) {
//		FileLog("httpclient", "JNI_OnLoad( ssl_version : %s )", data->ssl_version);
//	}
//
//	sh = curl_share_init();
...
void HttpClient::SetLogDirectory(string directory) {
	...
	FileLog("httpclient", "SetLogDirectory( directory : %s )", directory.c_str());

	curl_version_info_data *data = curl_version_info(CURLVERSION_FIRST);

	if( data->version != NULL ) {
		FileLog("httpclient", "SetLogDirectory( curl_version : %s )", data->version);
	}

	if( data->ssl_version != NULL ) {
		FileLog("httpclient", "SetLogDirectory( ssl_version : %s )", data->ssl_version);
	}
}
