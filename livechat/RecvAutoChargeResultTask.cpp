 ...
 * author: Samson.Fan
 *   date: 2015-04-01
 *   file: RecvAutoChargeResultTask.cpp
...
RecvAutoChargeResultTask::RecvAutoChargeResultTask(void)
{
	m_listener = NULL;
...
// 初始化
bool RecvAutoChargeResultTask::Init(ILiveChatClientListener* listener)
{
	bool result = false;
	if (NULL != listener)
	{
...
// 处理已接收数据
bool RecvAutoChargeResultTask::Handle(const TransportProtocol* tp)
{
	bool result = false;
	AmfParser parser;
...
bool RecvAutoChargeResultTask::GetSendData(void* data, unsigned int dataSize, unsigned int& dataLen)
{
	bool result = false;
	// 本协议没有返回
	return result;
...
// 获取待发送数据的类型
TASK_PROTOCOL_TYPE RecvAutoChargeResultTask::GetSendDataProtocolType()
{
	return JSON_PROTOCOL;
}
	
// 获取命令号
int RecvAutoChargeResultTask::GetCmdCode()
{
	return TCMD_RECVAUTOCHARGE;
}

// 设置seq
void RecvAutoChargeResultTask::SetSeq(unsigned int seq)
{
	m_seq = seq;
}

// 获取seq
unsigned int RecvAutoChargeResultTask::GetSeq()
{
	return m_seq;
}

// 是否需要等待回复。若false则发送后释放(delete掉)，否则发送后会被添加至待回复列表，收到回复后释放
bool RecvAutoChargeResultTask::IsWaitToRespond()
{
	return false;
}

// 获取处理结果
void RecvAutoChargeResultTask::GetHandleResult(LCC_ERR_TYPE& errType, string& errMsg)
{
	errType = m_errType;
	errMsg = m_errMsg;
}
...
// 未完成任务的断线通知
void RecvAutoChargeResultTask::OnDisconnect()
{
	// 不用回调
}
