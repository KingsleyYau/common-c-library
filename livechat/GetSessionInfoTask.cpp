...
GetSessionInfoTask::GetSessionInfoTask(void)
{
	m_listener = NULL;
...
// 初始化
bool GetSessionInfoTask::Init(ILiveChatClientListener* listener)
{
	bool result = false;
	if (NULL != listener)
	{
...
// 处理已接收数据
bool GetSessionInfoTask::Handle(const TransportProtocol* tp)
{
	bool result = false;
...
bool GetSessionInfoTask::GetSendData(void* data, unsigned int dataSize, unsigned int& dataLen)
{
	bool result = false;
...
// 获取待发送数据的类型
TASK_PROTOCOL_TYPE GetSessionInfoTask::GetSendDataProtocolType()
{
	return JSON_PROTOCOL;
}

// 获取命令号
int GetSessionInfoTask::GetCmdCode()
{
	return TCMD_GETSESSIONINFO;
}

// 设置seq
void GetSessionInfoTask::SetSeq(unsigned int seq)
{
	m_seq = seq;
}

// 获取seq
unsigned int GetSessionInfoTask::GetSeq()
{
	return m_seq;
}

// 是否需要等待回复。若false则发送后释放(delete掉)，否则发送后会被添加至待回复列表，收到回复后释放
bool GetSessionInfoTask::IsWaitToRespond()
{
	return true;
}

// 获取处理结果
void GetSessionInfoTask::GetHandleResult(LCC_ERR_TYPE& errType, string& errMsg)
{
	errType = m_errType;
	errMsg = m_errMsg;
}
...
// 初始化参数
bool GetSessionInfoTask::InitParam(const string& userId)
{
	bool result = false;
	if (!userId.empty()) {
		m_userId = userId;
...
// 未完成任务的断线通知
void GetSessionInfoTask::OnDisconnect()
{
	if (NULL != m_listener) {
		SessionInfoItem item;
		m_listener->OnGetSessionInfo(m_userId, LCC_ERR_CONNECTFAIL, "", item);
	}
}
