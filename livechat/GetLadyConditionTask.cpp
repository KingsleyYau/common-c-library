...
GetLadyConditionTask::GetLadyConditionTask(void)
{
	m_listener = NULL;
...
// 初始化
bool GetLadyConditionTask::Init(ILiveChatClientListener* listener)
{
	bool result = false;
	if (NULL != listener)
	{
...
// 处理已接收数据
bool GetLadyConditionTask::Handle(const TransportProtocol* tp)
{
	bool result = false;
...
bool GetLadyConditionTask::GetSendData(void* data, unsigned int dataSize, unsigned int& dataLen)
{
	bool result = false;
...
// 获取待发送数据的类型
TASK_PROTOCOL_TYPE GetLadyConditionTask::GetSendDataProtocolType()
{
	return JSON_PROTOCOL;
}
	
// 获取命令号
int GetLadyConditionTask::GetCmdCode()
{
	return TCMD_GETLADYCONDITION;
}

// 设置seq
void GetLadyConditionTask::SetSeq(unsigned int seq)
{
	m_seq = seq;
}

// 获取seq
unsigned int GetLadyConditionTask::GetSeq()
{
	return m_seq;
}

// 是否需要等待回复。若false则发送后释放(delete掉)，否则发送后会被添加至待回复列表，收到回复后释放
bool GetLadyConditionTask::IsWaitToRespond()
{
	return true;
}

// 获取处理结果
void GetLadyConditionTask::GetHandleResult(LCC_ERR_TYPE& errType, string& errMsg)
{
	errType = m_errType;
	errMsg = m_errMsg;
}
...
// 初始化参数
bool GetLadyConditionTask::InitParam(const string& userId)
{
	bool result = false;
	if (!userId.empty()) {
		m_userId = userId;
...
// 未完成任务的断线通知
void GetLadyConditionTask::OnDisconnect()
{
	if (NULL != m_listener) {
		LadyConditionItem item;
		m_listener->OnGetLadyCondition(m_userId, LCC_ERR_CONNECTFAIL, "", item);
	}
}
