...
SendLadyVideoTask::SendLadyVideoTask(void)
{
	m_listener = NULL;
...
// 初始化
bool SendLadyVideoTask::Init(ILiveChatClientListener* listener)
{
	bool result = false;
	if (NULL != listener)
	{
...
// 处理已接收数据
bool SendLadyVideoTask::Handle(const TransportProtocol* tp)
{
	bool result = false;
...
bool SendLadyVideoTask::GetSendData(void* data, unsigned int dataSize, unsigned int& dataLen)
{
	bool result = false;
...
// 获取待发送数据的类型
TASK_PROTOCOL_TYPE SendLadyVideoTask::GetSendDataProtocolType()
{
	return JSON_PROTOCOL;
}
	
// 获取命令号
int SendLadyVideoTask::GetCmdCode()
{
	return TCMD_SENDLADYVIDEO;
}

// 设置seq
void SendLadyVideoTask::SetSeq(unsigned int seq)
{
	m_seq = seq;
}

// 获取seq
unsigned int SendLadyVideoTask::GetSeq()
{
	return m_seq;
}

// 是否需要等待回复。若false则发送后释放(delete掉)，否则发送后会被添加至待回复列表，收到回复后释放
bool SendLadyVideoTask::IsWaitToRespond()
{
	return true;
}

// 获取处理结果
void SendLadyVideoTask::GetHandleResult(LCC_ERR_TYPE& errType, string& errMsg)
{
	errType = m_errType;
	errMsg = m_errMsg;
}
...
// 初始化参数
bool SendLadyVideoTask::InitParam(const string& userId, const string& inviteId, const string& videoId, const string& sendId, bool charget, const string& videoDesc, int ticket)
{
	bool result = false;
	if (!userId.empty() 
		&& !inviteId.empty()
...
// 未完成任务的断线通知
void SendLadyVideoTask::OnDisconnect()
{
	if (NULL != m_listener) {
		m_listener->OnSendLadyVideo(LCC_ERR_CONNECTFAIL, "", m_ticket);
	}
}

// 构建video参数值
string SendLadyVideoTask::GetVideoValue(const string& videoId, const string& sendId, bool charget, const string& videoDesc)
{
	string video("");
...
// 解析video参数
bool SendLadyVideoTask::ParsingVideoValue(const string& video, string& videoId, string& sendId, bool& charget, string& videoDesc)
{
	bool result = false;
	size_t begin = 0;
	size_t end = 0;
