 ...
 * author: Samson.Fan
 *   date: 2015-03-25
 *   file: GetLadyCustomTemplateTask.cpp
...
GetLadyCustomTemplateTask::GetLadyCustomTemplateTask(void)
{
	m_listener = NULL;
...
// 初始化
bool GetLadyCustomTemplateTask::Init(ILiveChatClientListener* listener)
{
	bool result = false;
	if (NULL != listener)
	{
...
// 处理已接收数据
bool GetLadyCustomTemplateTask::Handle(const TransportProtocol* tp)
{
	bool result = false;
...
bool GetLadyCustomTemplateTask::GetSendData(void* data, unsigned int dataSize, unsigned int& dataLen)
{
	bool result = false;
...
// 获取待发送数据的类型
TASK_PROTOCOL_TYPE GetLadyCustomTemplateTask::GetSendDataProtocolType()
{
	return JSON_PROTOCOL;
}
	
// 获取命令号
int GetLadyCustomTemplateTask::GetCmdCode()
{
	return TCMD_GETLADYCUSTOMTEMPLATE;
}

// 设置seq
void GetLadyCustomTemplateTask::SetSeq(unsigned int seq)
{
	m_seq = seq;
}

// 获取seq
unsigned int GetLadyCustomTemplateTask::GetSeq()
{
	return m_seq;
}

// 是否需要等待回复。若false则发送后释放(delete掉)，否则发送后会被添加至待回复列表，收到回复后释放
bool GetLadyCustomTemplateTask::IsWaitToRespond()
{
	return true;
}

// 获取处理结果
void GetLadyCustomTemplateTask::GetHandleResult(LCC_ERR_TYPE& errType, string& errMsg)
{
	errType = m_errType;
	errMsg = m_errMsg;
}
...
// 初始化参数
bool GetLadyCustomTemplateTask::InitParam(const string& userId)
{
	bool result = false;
	if (!userId.empty()) {
		m_userId = userId;
...
// 未完成任务的断线通知
void GetLadyCustomTemplateTask::OnDisconnect()
{
	if (NULL != m_listener) {
		vector<string> contents;
		vector<bool> flags;
		m_listener->OnGetLadyCustomTemplate(m_userId, LCC_ERR_CONNECTFAIL, "", contents, flags);
	}
}
