...
class LCMagicIconManagerCallback
{
public:
	LCMagicIconManagerCallback() {};
	virtual ~LCMagicIconManagerCallback() {};
public:
	virtual void OnDownloadMagicIconImage(bool result, LCMagicIconItem* magicIconItem) = 0;
	virtual void OnDownloadMagicIconThumbImage(bool result, LCMagicIconItem* magicIconItem) = 0;
};

class LCMessageItem;
class LCMagicIconDownloader;
class LCMagicIconManager : private LCMagicIconDownloaderCallback
{
private:
	// <magicIconId, item>
	typedef map<string, LCMagicIconItem*> MagicIconMap;
	// <msgId, LCMessageItem>
	typedef map<int, LCMessageItem*> ToSendMap;
	// <magicIconId, downloader>
	typedef map<string, LCMagicIconDownloader*> MagicIconDownloaderMap;
private:
	// 已下载完成的downloader（待释放）
	typedef struct _tagFinishDownloaderItem {
		LCMagicIconDownloader*	downloader;
...
public:
	LCMagicIconManager();
	virtual ~LCMagicIconManager();

public:
	// 设置本地缓存目录路径
	bool SetDirPath(const string& dirPath, const string& logPath);
	// 初始化
	bool Init(const string& host, LCMagicIconManagerCallback* callback);
	// 设置http认证帐号
...
public:
	...
	// 更新配置
	bool UpdateConfigItem(const MagicIconConfig& configItem);
	// 获取配置item
	MagicIconConfig GetConfigItem() const;
private:
	// 从文件中获取配置item
...
public:
	// 获取/添加小高级表情item
	LCMagicIconItem* GetMagicIcon(const string& MagicIconId);
private:
	// 添加配置item的小高级表情至map （批量）
	void AddMagicIconWithConfigItem(const MagicIconConfig& configItem);
	// 批量添加高级表情至map（放弃）
	//void AddEmotions(const OtherEmotionConfigItem::EmotionList& emotionList);
	// 添加小高级表情至map
	bool AddMagicIconItem(LCMagicIconItem* item);
	// 清除所有小高级表情item
...
public:
	// 开始下载原图image
	bool StartDownloadSourceImage(LCMagicIconItem* item);
	// 停止下载原图image
	bool StopDownloadSourceImage(LCMagicIconItem* item);
	// 停止所有原图image下载
...
private:
	...
	void AddToFinishDownloaderList(LCMagicIconDownloader* downloader);

	// ------------ 下载Thumbimage ------------
public:
	// 开始下载拇子图image
	bool StartDownloadThumbImage(LCMagicIconItem* item);
	// 停止下载拇子图image
	bool StopDownloadThumbImage(LCMagicIconItem* item);
	// 停止下载所有拇子图image
...
private:
	virtual void onSuccess(LCMagicIconDownloader* downloader, LCMagicIconDownloader::MagicIconDownloadType downloadType, LCMagicIconItem* item);
	virtual void onFail(LCMagicIconDownloader* downloader, LCMagicIconDownloader::MagicIconDownloadType downloadType, LCMagicIconItem* item);
...
private:
	...
	MagicIconConfig	m_configItem;			// 小高级表情配置item
	LCMagicIconManagerCallback*	m_callback;	// 回调
