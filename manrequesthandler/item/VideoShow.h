...
typedef list<VSVideoListItem> VSVideoList;

// 查询指定女士的视频信息 VS_VIDEODETAIL_PATH(/member/video_detail)
class VSVideoDetailItem {
public:
	bool Parsing(Json::Value root) {
		bool result = false;
		if( root.isObject() ) {
...
typedef list<VSVideoDetailItem> VSVideoDetailList;

// 查询视频详细信息 VS_PLAYVIDEO_PATH(/member/play_video)
class VSPlayVideoItem {
public:
	bool Parsing(Json::Value root) {
		bool result = false;
		if( root.isObject() ) {
...
public:
	...
	virtual ~VSWatchedVideoListItem() {}
...
typedef list<VSWatchedVideoListItem> VSWatchedVideoList;


// 查询已收藏的视频列表 VS_SAVEDVIDEO_PATH(/member/saved_video)
class VSSavedVideoListItem {
public:
	bool Parsing(Json::Value root) {
		bool result = false;
		if( root.isObject() ) {
...
typedef list<VSSavedVideoListItem> VSSavedVideoList;
